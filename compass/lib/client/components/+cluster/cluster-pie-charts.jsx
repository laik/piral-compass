...
export const ClusterPieCharts = observer(() => {
    const renderLimitWarning = () => {
        return (<div className="node-warning flex gaps align-center">
        <Icon material="info"/>
    ...
    const renderCharts = () => {
        ...
        return (<div className="NodeCharts flex justify-center box grow gaps">
        <div className="chart flex column align-center box grow">
          <PieChart data={cpuData} title={`CPU`} legendColors={["#c93dce", "#4caf50", "#3d90ce", defaultColor]}/>
          {cpuLimitsOverload && renderLimitWarning()}
        </div>
        <div className="chart flex column align-center box grow">
          <PieChart data={memoryData} title={`Memory`} legendColors={["#c93dce", "#4caf50", "#3d90ce", defaultColor]}/>
    ...
    const renderContent = () => {
        ...
        if (!nodes.length) {
            return (<div className="empty flex column box grow align-center justify-center">
          <Icon material="info"/>
          No Nodes Available.
        </div>);
        }
        if (!metricsLoaded) {
            return (<div className="flex justify-center align-center box grow empty">
          <Spinner />
        </div>);
        }
    ...
    return (<div className="ClusterPieCharts flex">
      {renderContent()}
    </div>);
});
//# sourceMappingURL=cluster-pie-charts.jsx.map