...
export const ClusterMetrics = observer(() => {
    ...
    const memoryOptions = {
        ...
        tooltips: {
            callbacks: {
                label: ({ index }, data) => {
                    const value = data.datasets[0].data[index];
                    return bytesToUnits(parseInt(value.y), 3);
                }
            }
        }
    };
    ...
    const renderMetrics = () => {
        ...
        return (<BarChart name={`${metricNodeRole}-${metricType}`} options={options} data={{ datasets }} timeLabelStep={5} showLegend={false} plugins={[ZebraStripes]}/>);
    };
    return (<div className="ClusterMetrics flex column">
      <ClusterMetricSwitchers />
      {renderMetrics()}
    </div>);
});
//# sourceMappingURL=cluster-metrics.jsx.map