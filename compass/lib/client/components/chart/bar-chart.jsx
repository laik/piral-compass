...
export function BarChart(props) {
    const { name, data, className, timeLabelStep, plugins, options: customOptions } = props, settings = __rest(props, ["name", "data", "className", "timeLabelStep", "plugins", "options"]);
    ...
    const chartData = Object.assign(Object.assign({}, data), { datasets: data.datasets
            ...
            .map(item => {
            return Object.assign({ type: ChartKind.BAR, borderWidth: { top: 3 }, barPercentage: 1, categoryPercentage: 1 }, item);
        }) });
    ...
    return (<Chart className={cssNames("BarChart flex box grow column", className)} type={ChartKind.BAR} data={chartData} options={options} plugins={plugins} {...settings}/>);
}
...
//# sourceMappingURL=bar-chart.jsx.map